import { ProxyRotatorClient } from "./client";
import { buildProxyUsername } from "./types";
import type { SessionMetadata } from "./types";

let _client: ProxyRotatorClient | null = null;

export interface ProxyConfig {
    /** Base URL of the proxy-rotator, e.g. "http://proxy-rotator:8100" */
    proxyUrl: string;
    /** API key for authenticating with the proxy-rotator management API */
    apiKey: string;
}

/**
 * Configure the proxy-rotator client once at startup.
 *
 * After calling this, `buildAndVerifyProxyUsername` can be used to build and
 * verify usernames against the proxy-rotator before use.
 *
 * @example
 * configureProxy({ proxyUrl: PROXY_URL, apiKey: PROXY_API_KEY });
 */
export function configureProxy(opts: ProxyConfig): void {
    _client = new ProxyRotatorClient({ baseUrl: opts.proxyUrl, apiKey: opts.apiKey });
}

/**
 * Build the proxy-rotator username — a base64-encoded JSON object — and
 * verify it against the proxy-rotator before returning.
 *
 * Verification checks:
 * 1. The proxy set exists in the proxy-rotator config
...
export async function buildAndVerifyProxyUsername(
    ...
    if (!_client) {
        throw new Error("buildAndVerifyProxyUsername: proxy client not configured. Call configureProxy() at startup.");
    }
