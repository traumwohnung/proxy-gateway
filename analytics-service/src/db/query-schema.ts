...
export function validate(
  input: unknown,
): { ok: true; query: UsageQuery } | { ok: false; errors: QueryError[] } {
  ...
  if (q.time.resolution) {
    ...
    if (buckets > MAX_BUCKETS) {
      errors.push({
        error: `resolution too fine: would produce ${buckets} buckets (max ${MAX_BUCKETS})`,
  ...
  if (q.sort) {
    ...
    if (
      q.sort.by !== 'metric' &&
      q.sort.by !== 'time' &&
      (!q.group_by || q.group_by.length === 0)
    ) {
