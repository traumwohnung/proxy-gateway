...
mod tests {
    ...
    fn test_parse_host_port_user_pass() {
        let p = parse_proxy_line("150.241.111.154:6658:myuser:mypass123").unwrap();
        assert_eq!(p.host, "150.241.111.154");
    ...
    fn test_load_proxies_mixed() {
        ...
        writeln!(f, "150.241.111.154:6658:myuser:mypass").unwrap();
        writeln!(f, "").unwrap();
        writeln!(f, "82.23.88.116:7872:myuser:mypass").unwrap();
        writeln!(f, "plain.proxy.com:3128").unwrap();
        f.flush().unwrap();

        let proxies = load_proxies(f.path()).unwrap();
        assert_eq!(proxies.len(), 3);
        assert_eq!(proxies[0].host, "150.241.111.154");
