...
// OnEpochTransition forwards the event to analytics. ParamsJSON rides only
// on first_bind, matching the proto contract; the analytics server uses it
// to backfill session_params_dim.params_json.
func (l *analyticsEpochListener) OnEpochTransition(ev utils.EpochEvent) {
	...
	l.client.SendEpochTransition(analytics.EpochTransition{
		...
		SessionParamsHash: ev.SessionParamsHash,
